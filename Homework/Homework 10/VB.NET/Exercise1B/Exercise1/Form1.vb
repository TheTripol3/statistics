Public Class Form1
    ...
    Private Theta As Double = Nothing
    Private eqLevel As Double = Nothing
    Private Δt As Double = Nothing

    Dim Exp_θΔt As Double = Nothing
    Dim Exp_2θΔt As Double = Nothing
    Dim One_MinusExp_θΔt As Double = Nothing
    Dim One_MinusExp_2θΔt As Double = Nothing
    ...
    Dim intervalSizeN As Double = 0.2
    Dim startingEndPointN As Double = 0.5
        ...
        If IsNumeric(TextBox1.Text) AndAlso (Convert.ToInt32(TextBox1.Text) > 0) Then
            If IsNumeric(TextBox2.Text) AndAlso (Convert.ToInt32(TextBox2.Text) > 0) Then
                If IsNumeric(TextBox3.Text) AndAlso (Convert.ToDouble(TextBox3.Text) > 0) Then
                    ...
                    initial = Convert.ToDouble(TextBox4.Text)
                    eqLevel = Convert.ToDouble(TextBox5.Text)
                    Theta = Convert.ToDouble(TextBox6.Text)
                    Δt = 1 / n

                    Exp_θΔt = Math.Exp(-Theta * Δt)
                    Exp_2θΔt = Math.Exp(-2 * Theta * Δt)
                    One_MinusExp_θΔt = 1 - Exp_θΔt
                    One_MinusExp_2θΔt = 1 - Exp_2θΔt
    ...
    Private Sub generation()
        ...
        For i As Integer = 1 To m
            ...
            For j As Integer = 1 To n
                ...
                precedent = precedent * Exp_θΔt + Me.eqLevel * One_MinusExp_θΔt + sigma * Math.Sqrt(One_MinusExp_2θΔt / (2 * Me.Theta)) * numberZi
                'Me.RichTextBox1.AppendText(precedent & Environment.NewLine)
    ...
    Private Sub DrawSceneGenerate()
        ...
        Dim Y_Line As Single = Me.Y_ViewPort(MaxY_windows * Math.Truncate(ViewPort.Height / (PictureBox1.Height)), ViewPort, MinY_windows, RangeY)
        ...
        For Each objectList In listDictNFinal
            ...
            Y_Previous = Me.Y_ViewPort(0 * Math.Truncate(ViewPort.Height / (20)), ViewPort, MinY_windows, RangeY)
            ...
            For Each kvp As KeyValuePair(Of Integer, Double) In objectList
                ...
                Dim Y_Device As Single = Me.Y_ViewPort(kvp.Value * Math.Truncate(ViewPort.Height / (20)), ViewPort, MinY_windows, RangeY)
        ...
        For Each kvp As sizeIntervals In listIntervalsN2
            Dim freq As Double = (kvp.countInt / ((n / 2) * m)) * 200

            Dim XL As Single = Me.X_ViewPort((n / 2) * (ViewPort.Width / (n + Convert.ToInt32(TextBox1.Text / 4))), ViewPort, MinX_windows, RangeX)
            Dim YL2 As Single = Me.Y_ViewPort(kvp.upperPoint * Math.Truncate(ViewPort.Height / 20), ViewPort, MinY_windows, RangeY)
            Dim YL1 As Single = Me.Y_ViewPort(kvp.lowerPoint * Math.Truncate(ViewPort.Height / 20), ViewPort, MinY_windows, RangeY)

            Dim X2L As Single = Me.X_ViewPort(ViewPort.Width, ViewPort, MinX_windows, RangeX)

            Dim height As Single = Me.Y_ViewPort(kvp.lowerPoint * Math.Truncate(ViewPort.Height / 20), ViewPort, MinY_windows, RangeY) - Me.Y_ViewPort(kvp.upperPoint * Math.Truncate(ViewPort.Height / 20), ViewPort, MinY_windows, RangeY)
