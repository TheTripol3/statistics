Public Class Form1
    ...
    Function findRange(randomNumber As Double, listIntv As List(Of sizeIntervals))
        ...
        If ((listIntv(0).lowerPoint < randomNumber) AndAlso (randomNumber <= listIntv(Math.Ceiling(listIntv.Count / 2)).lowerPoint)) Then
            ...
            If (Math.Abs(listIntv(0).lowerPoint - randomNumber) <= Math.Abs(randomNumber - listIntv(Math.Ceiling(listIntv.Count / 2)).lowerPoint)) Then
                Return "-L"
        ...
        ElseIf ((listIntv(Math.Ceiling(listIntv.Count / 2)).lowerPoint < randomNumber) AndAlso (randomNumber <= listIntv(listIntv.Count - 1).upperPoint)) Then
            ...
            If (Math.Abs(listIntv(Math.Ceiling(listIntv.Count / 2)).lowerPoint - randomNumber) <= Math.Abs(randomNumber - listIntv(listIntv.Count - 1).upperPoint)) Then
                Return "-R"
    ...
    Private Sub calculateContinuousDistribution(sizeInterval As Double,
        ...
        Select Case range
            ...
            Case "+L"
                ...
                For index As Integer = (Math.Ceiling((listInt.Count / 2)) - 1) To 0 Step -1
                    If ((valuesC > listInt(index).lowerPoint) AndAlso (valuesC <= listInt(index).upperPoint)) Then
                        listInt(index).countInt += 1
                        Exit Sub
                    End If
            ...
            Case "+R"
                ...
                For index As Integer = (listInt.Count - 1) To Math.Ceiling((listInt.Count / 2)) Step -1
                    If ((valuesC > listInt(index).lowerPoint) AndAlso (valuesC <= listInt(index).upperPoint)) Then
                        listInt(index).countInt += 1
                        Exit Sub
                    End If
